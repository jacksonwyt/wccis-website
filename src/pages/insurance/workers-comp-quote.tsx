...
const WorkersCompQuotePage = () => {
  ...
  const onSubmit = (data: WorkersCompQuote) => {
    ...
    try {
      ...
      const subject = `Workers Comp Quote Request - License ${data.contractorsLicenseNumber || 'N/A'}`;
      
      let body = 'Workers Compensation Insurance Quote Request:\n\n';
      for (const key in data) {
        if (Object.prototype.hasOwnProperty.call(data, key)) {
          const typedKey = key as keyof WorkersCompQuote;
          const label = typedKey.replace(/([A-Z])/g, ' $1').replace(/^./, (str) => str.toUpperCase());
          body += `${label}: ${data[typedKey] || 'Not provided'}\n`;
        }
      }
      body = body.trim();
