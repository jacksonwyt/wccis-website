...
const wcQuoteSchema = z.object({
  ...
  grossAnnualReceipts: z.number().min(0),
  subcontractorCosts: z.number().min(0),
  subcontractorPercentage: z.number().min(0).max(100),
  
  totalOwners: z.number().min(0),
...
const WorkersCompQuotePage = () => {
  ...
  const { submit, isSubmitting, error } = useFormSubmit({
    submitFn: async (data: WCQuoteForm) => {
